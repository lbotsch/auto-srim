...
class Collision:
    ...
    def _read_ion(self, ion_str):
        ...
        while True:
            try:
                line = last_line if last_line is not None else next(lines)
                last_line = None

                if re.match("^=+\r$", line):
                    break

                # Skip lines until the start of a new cascade
                while not "<== Start of New Cascade" in line:
                    if "For All Ions to date" in line:
                        end_of_ion = True
                        break
                    line = next(lines)
                if end_of_ion:
        ...
        # Reads ion footer
        while not "For All Ions to date" in line:
            line = next(lines)

        ion_number = re.search(int_regex, line).group(0)
    ...
    def _read_cascade(self, lines):
        ...
        cascade = []
        for line in lines:
            # Strange case where the cascade summary is missing
            if "<== Start of New Cascade" in line or line.count("=") > 100:
                return (line,)
